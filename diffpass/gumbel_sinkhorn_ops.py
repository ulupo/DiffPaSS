...
# %% auto 0
__all__ = ['inverse_permutation_arr', 'randperm_mat_like', 'randperm_arr_like', 'unbias_by_randperms_mat',
           'unbias_by_randperms_arr', 'gumbel_noise_like', 'sinkhorn_norm', 'log_sinkhorn_norm', 'gumbel_sinkhorn',
           'np_matching', 'matching', 'gumbel_matching', 'inverse_permutation']
...
def inverse_permutation_arr(perm: torch.Tensor) -> torch.Tensor:
...
def randperm_arr_like(log_alpha: torch.Tensor) -> torch.Tensor:
    """Generate a 1D random permutation array with the same length as ``log_alpha[-2]``.
    Assume log_alpha is of shape (*batch_size, n, n)."""
    n = log_alpha.size()[-2]
...
def unbias_by_randperms_mat(func: callable) -> callable:
...
def unbias_by_randperms_arr(func: callable) -> callable:
    """Decorator to unbias `func` with two random permutations."""

    def wrapper(log_alpha: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        # Create two 1D random permutation arrays
        rand_perm_arrs = [randperm_arr_like(log_alpha), randperm_arr_like(log_alpha)]
        # Conjugate log_alpha with the two random permutations
        log_alpha_conj = log_alpha[..., :, rand_perm_arrs[1]][..., rand_perm_arrs[0], :]
        # Apply the function
        out_conj = func(log_alpha_conj, *args, **kwargs)
        # Compute inverse permutation 1D arrays and conjugate back
        inv_rand_perm_arrs = [inverse_permutation_arr(rp) for rp in rand_perm_arrs]
        out_unconj = out_conj[..., :, inv_rand_perm_arrs[1]][
            ..., inv_rand_perm_arrs[0], :
        ]
        return out_unconj
...
def gumbel_matching(
    ...
    *,
    ...
    gumbel_matching_impl = unbias_by_randperms_arr(matching) if unbias_lsa else matching
